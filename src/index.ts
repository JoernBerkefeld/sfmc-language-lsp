...
import type { Diagnostic, CompletionItem, Hover, SignatureHelp, Location, CodeAction, Position } from './types.js';
...
import { getAmpscriptCompletions, resolveAmpscriptCompletion, functionCompletionItems } from './completions/ampscript.js';
...
export type { SsjsFunction, SsjsFunctionParam, EcmascriptBuiltin, SsjsObject } from './data/ssjs.js';
...
export class SfmcLanguageService {
    ...
    getUnsupportedSsjsSyntax(): Array<{ pattern: string; message: string }> {
        return [
            { pattern: 'let/const', message: "'let'/'const' declarations are not supported in SFMC SSJS. Use 'var' instead." },
            { pattern: '=>', message: 'Arrow functions are not supported in SFMC SSJS. Use a regular function expression.' },
            { pattern: '`template literals`', message: 'Template literals are not supported in SFMC SSJS. Use string concatenation.' },
            { pattern: 'class', message: 'Class declarations are not supported in SFMC SSJS. Use constructor functions.' },
